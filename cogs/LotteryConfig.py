...
class LotteryConfig(commands.Cog):
    ...
    async def lottery_settings(self, ctx):
        ...
        cmds = [
            (f"`{ctx.prefix}뽑기설정 경품목록`", "현재 경품 구성을 확인합니다."),
            (f"`{ctx.prefix}뽑기설정 경품추가 (경품명)`", "경품을 추가합니다."),
            (f"`{ctx.prefix}뽑기설정 경품셔플`", "경품 번호를 랜덤 배정합니다."),
            (f"`{ctx.prefix}뽑기설정 경품초기화`", "경품을 꽝 100개로 초기화합니다."),
            (f"`{ctx.prefix}뽑기설정 알림채널설정`", "뽑기 결과 알림 채널을 설정합니다."),
            (f"`{ctx.prefix}뽑기설정 역할설정`", "당첨 시 멘션할 역할을 설정합니다."),
            (f"`{ctx.prefix}뽑기설정 뽑기판생성`", "현재 채널에 뽑기판을 생성합니다."),
            (f"`{ctx.prefix}뽑기설정 메시지생성`", "현재 채널에 뽑기권 안내 메시지를 생성합니다."),
            (f"`{ctx.prefix}뽑기설정 자동초기화 [true/false]`", "뽑기판 모두 소모 시 자동 초기화 여부를 설정합니다."),
        ]
    ...
    async def create_board(self, ctx):
        ...
        if not gc.get("shuffled"):
            await ctx.send(f"⚠️ 먼저 `{ctx.prefix}뽑기설정 경품셔플`을 실행해주세요.")
            return
